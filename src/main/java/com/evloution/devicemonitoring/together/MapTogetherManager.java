...
import com.evloution.devicemonitoring.utils.DisplayUtil;
...
public class MapTogetherManager {
    ...
    private void assignSingleCluster(Marker marker) {
        MarketBean dotInfo = (MarketBean) marker.getObject();
        LatLng latLng = new LatLng(dotInfo.getDeviceLatitude(), dotInfo.getDeviceLongitude());
        ...
        if (togMarketBean != null) {
            ...
            togMarketBean.addClusterItem(marker);
            togDotInfoMap.put(dotInfo.getDeviceIP() + SystemClock.currentThreadTimeMillis(), togMarketBean);
        }
    }
